...
void RaftNode::RequestPreVote() {
    ...
    RAFTCPP_LOG(RLL_DEBUG)<<"Node "<<this->config_.GetThisEndpoint().ToString()<<" request prevote";
    ...
    for (const auto &rpc_client : rpc_clients_) {
        RAFTCPP_LOG(RLL_DEBUG)<<"RequestPreVote Node "<< this->config_.GetThisEndpoint().ToString() <<"request_vote_callback client"<<rpc_client.get();
...
void RaftNode::OnRequestPreVote(rpc::RpcConn conn, const std::string &endpoint_str,
    ...
    RAFTCPP_LOG(RLL_DEBUG) <<"OnRequestPreVote this node " <<this->config_.GetThisEndpoint().ToString()<< " Received a RequestPreVote from node " << endpoint_str
                           << " term_id=" << term_id;
    ...
    if (curr_state_ == RaftState::FOLLOWER) {
        if (term_id > curr_term_id_.getTerm()) {
            ...
            timer_manager_.GetElectionTimerRef().Reset(
                    2000+randomer_.TakeOne(1000, 2000));
            if (conn_sp) {
...
void RaftNode::OnPreVote(const boost::system::error_code &ec, string_view data) {
    ...
    RAFTCPP_LOG(RLL_DEBUG) << "OnPreVote Response node： "<<data.data()<<" this node:"<<this->config_.GetThisEndpoint().ToString();
    std::lock_guard<std::recursive_mutex> guard{mutex_};
    ...
    if (this->config_.GreaterThanHalfNodesNum(responded_pre_vote_nodes_.size()) &&
        ...
        RAFTCPP_LOG(RLL_INFO) << "This node "<<this->config_.GetThisEndpoint().ToString()<<" has became a candidate now.";
        curr_term_id_.setTerm(curr_term_id_.getTerm() + 1);
...
void RaftNode::RequestVote() {
    std::lock_guard<std::recursive_mutex> guard{mutex_};
    RAFTCPP_LOG(RLL_DEBUG)<<"Node "<<this->config_.GetThisEndpoint().ToString()<<" request vote";
...
void RaftNode::OnRequestVote(rpc::RpcConn conn, const std::string &endpoint_str,
                             int32_t term_id) {
    RAFTCPP_LOG(RLL_DEBUG)<<"Node "<<this->config_.GetThisEndpoint().ToString()<<" response vote";
    std::lock_guard<std::recursive_mutex> guard{mutex_};
    const auto req_id = conn.lock()->request_id();
    auto conn_sp = conn.lock();
    if (curr_state_ == RaftState::FOLLOWER) {
...
void RaftNode::OnVote(const boost::system::error_code &ec, string_view data) {
    if (ec.message() == "Transport endpoint is not connected") return;
    //Exclude itself under multi-thread
    RAFTCPP_LOG(RLL_DEBUG) << "OnVote Response node： "<<data.data()<<" this node:"<<this->config_.GetThisEndpoint().ToString();
    std::lock_guard<std::recursive_mutex> guard{mutex_};
    ...
    if (this->config_.GreaterThanHalfNodesNum(responded_vote_nodes_.size()) &&
        ...
        RAFTCPP_LOG(RLL_INFO) << "This node "<<this->config_.GetThisEndpoint().ToString()<<" has became a leader now";
        curr_term_id_.setTerm(curr_term_id_.getTerm() + 1);
...
void RaftNode::RequestHeartbeat() {
    for (const auto &rpc_client : rpc_clients_) {
        RAFTCPP_LOG(RLL_DEBUG) << "Send a heartbeat to node.";
            auto request_heartbeat_callback = [this](const boost::system::error_code &ec, int32_t term_id) {
                //            io_service_.post([this, ec, data]() { this->OnVote(ec, data);
                //            });
...
void RaftNode::OnRequestHeartbeat(rpc::RpcConn conn, int32_t term_id)
{
    ...
    {
        RAFTCPP_LOG(RLL_DEBUG) <<"OnRequestHeartbeat node "<<this->config_.GetThisEndpoint().ToString()<< "received a heartbeat from leader."
                               << " curr_term_id_:"<<curr_term_id_.getTerm() << " receive term_id:"<<term_id<<" update term_id";
    ...
    else
    {
        if (term_id >= curr_term_id_.getTerm()) {
            RAFTCPP_LOG(RLL_DEBUG) <<"OnRequestHeartbeat node "<<this->config_.GetThisEndpoint().ToString()<< "received a heartbeat from leader."
                                   << " curr_term_id_:"<<curr_term_id_.getTerm() << " receive term_id:"<<term_id<<" StepBack";
            curr_term_id_.setTerm(term_id);
            timer_manager_.GetVoteTimerRef().Stop();
            ...
            RAFTCPP_LOG(RLL_DEBUG) <<"OnRequestHeartbeat node "<<this->config_.GetThisEndpoint().ToString()<< "received a heartbeat from leader and send response";
            std::lock_guard<std::recursive_mutex> guard{mutex_};
            const auto req_id = conn.lock()->request_id();
            auto conn_sp = conn.lock();
            if (conn_sp) {
...
void RaftNode::OnHeartbeat(const boost::system::error_code &ec, string_view data) {
    if (ec.message() == "Transport endpoint is not connected") return;
    RAFTCPP_LOG(RLL_DEBUG) << "Received a response heartbeat from node.term_id："<< std::stoi(data.data())<<"more than currentid:"<<curr_term_id_.getTerm();
    std::lock_guard<std::recursive_mutex> guard{mutex_};
...
void RaftNode::ConnectToOtherNodes() {
    ...
    for (const auto &endpoint : config_.GetOtherEndpoints()) {
        ...
        RAFTCPP_LOG(RLL_DEBUG) <<"This node "<<config_.GetThisEndpoint().ToString()<<" succeeded to connect to the node "
                               << endpoint.ToString();
