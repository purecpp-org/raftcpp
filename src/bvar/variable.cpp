...
//#include "macros.h"                        // BAIDU_CASSERT
//#include "butil/containers/flat_map.h"           // butil::FlatMap
//#include "butil/scoped_lock.h"                   // BAIDU_SCOPE_LOCK
//#include "string_splitter.h"               // butil::StringSplitter
//#include "string_split.h"          // butil::SplitStringIntoKeyValuePairs
#include "errno.h"                         // berror
#include "time.h"                          // milliseconds_from_now
//#include "file_util.h"                     // butil::FilePath
//#include "gflag.h"
#include "variable.h"
#include <map>
#include <mutex>
...
typedef std::map<std::string, VarEntry> VarMap;
...
// before main().
static std::once_flag s_var_maps_once;
...
inline VarMapWithLock* get_var_maps() {
    std::call_once(s_var_maps_once, init_var_maps);
    return s_var_maps;
}
...
int Variable::expose_impl(const std::string& prefix,
    ...
    {
        std::unique_lock<std::recursive_mutex> lock(m.mutex);
        VarEntry *entry = &(m.varMaps)[name];
        if (entry == NULL) {
            entry = &m[_name];
            entry->var = this;
...
bool Variable::hide() {
    ...
    std::unique_lock<std::recursive_mutex> lock(m.mutex);
    if (!m.varMaps.empty())
		m.varMaps.erase(_name);
    //VarEntry *entry = &(m.varMaps)[name];
    //VarEntry *entry = &(m.varMaps)[name];
    //if (entry) {
        //m.varMaps.erase(name);
...
void Variable::list_exposed(std::vector<std::string>* names,
    ...
    for (size_t i = 0; i < SUB_MAP_COUNT; ++i) {
        ...
        std::unique_lock<std::recursive_mutex> mu(m.mutex);
        size_t n = 0;
...
std::string Variable::get_description() {
