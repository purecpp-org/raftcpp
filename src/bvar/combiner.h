...
#include "linked_list.h"// LinkNode
#include "agent_group.h"    // detail::AgentGroup
...
public:
	ElementContainer() : _lock(_mutex, std::defer_lock)
	{
...
template <typename T>
class ElementContainer<
    T,typename std::atomic<T>::value::type > {
public:
    ...
    inline void store(T new_value) { _value.store(new_value, std::memory_order_relaxed);
    }
