...
public:
    ...
    inline static Agent *get_or_create_tls_agent(AgentId id) {
        ...
        if (block_id >= _s_tls_blocks->size()) {
            ...
            _s_tls_blocks->resize(std::max(static_cast<int>(block_id + 1), 32));
        }
...
private:
    ...
    static thread_local std::vector<ThreadBlock *> *_s_tls_blocks;
};
...
template <typename Agent>
thread_local std::vector<typename AgentGroup<Agent>::ThreadBlock *>
