...
namespace raftcpp {
    static spdlog::level::level_enum GetMappedSeverity(RaftcppLogLevel severity) {
        switch (severity) {
            case RaftcppLogLevel::RAFTCPP_TRACE:
                return spdlog::level::trace;
            case RaftcppLogLevel::RAFTCPP_DEBUG:
                return spdlog::level::debug;
            case RaftcppLogLevel::RAFTCPP_INFO:
                return spdlog::level::info;
            case RaftcppLogLevel::RAFTCPP_WARN:
                return spdlog::level::warn;
            case RaftcppLogLevel::RAFTCPP_ERROR:
                return spdlog::level::err;
            case RaftcppLogLevel::RAFTCPP_CRITICAL:
    ...
    RaftcppLog::RaftcppLog(const char *file_name, int line_number, RaftcppLogLevel severity) : filename_(file_name),
                                                                                               line_number_(
                                                                                                       line_number),
                                                                                               log_level_(std::move(
                                                                                                       severity)),
                                                                                               is_enabled_(severity >=
                                                                                                           severity_threshold_) {}

    RaftcppLog::~RaftcppLog() {
        try {
            if (is_enabled_)
                logging_provider->log(GetMappedSeverity(log_level_), "in {} line:{} {}", filename_, line_number_,
                                      ss_.str());
        }
    ...
    void
    RaftcppLog::StartRaftcppLog(const std::string &log_file_name, RaftcppLogLevel severity,
                                uint32_t log_file_roll_size_mb,
                                uint32_t log_file_roll_cout) {
        severity_threshold_ = severity;
        if (logging_provider == nullptr) {
            try {
                logging_provider = ::spdlog::rotating_logger_mt("raftcpp_log", log_file_name,
                                                                1024 * 1024 * 1024 * log_file_roll_size_mb,
                                                                log_file_roll_cout);
    ...
    void RaftcppLog::ShutDownRaftcppLog() {
        spdlog::shutdown();
    }
